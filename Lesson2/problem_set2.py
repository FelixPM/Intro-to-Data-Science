...
# print(num_rainy_days('weather_underground.csv.txt'))
...
# print(max_temp_aggregate_by_fog('weather_underground.csv.txt'))
...
def fix_turnstile_data(filenames):
    ...
    for name in filenames:
        file_in = open(name, 'r')
        file_out = open('updated_'+name, 'w', newline='')
        read_in = csv.reader(file_in, delimiter=',')
        writer_out = csv.writer(file_out)
        for line in read_in:
            first_three = line[:3]
            for row in range(3, len(line[4:]), 5):
                writer_out.writerow(first_three + line[row:row+5])
        file_in.close()
        file_out.close()
# your code here
